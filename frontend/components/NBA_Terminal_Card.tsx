...
interface NBAData {
  ...
  module_title?: string
  rolling_pct_10?: number
  attempts_last_10?: number
}

interface NBA_Terminal_CardProps {
  data: NBAData
  onExecute?: () => void
}

const PRIORITY_STYLES: Record<string, { color: string; tint: string; label: string; pulse: boolean }> = {
  CRITICAL: { color: '#ef4444', tint: 'rgba(239,68,68,.10)',  label: 'Critical', pulse: true  },
  HIGH:     { color: '#f59e0b', tint: 'rgba(245,158,11,.08)', label: 'High',     pulse: true  },
...
export const NBA_Terminal_Card = ({ data, onExecute }: NBA_Terminal_CardProps) => {
    ...
    <>
      <style jsx>{`
        @keyframes nba-pulse-ring {
          0%, 100% { box-shadow: 0 0 0 1px ${p.color}22, 0 0 28px ${p.color}22; }
          50%      { box-shadow: 0 0 0 1px ${p.color}66, 0 0 48px ${p.color}55; }
        }
      ...
      >
        {/* Left accent bar (urgency channel) */}
        <div className="absolute top-0 bottom-0 left-0 w-[4px] overflow-hidden" style={{ background: p.color }}>
          {p.pulse && (
            <div
        ...
        <div className="flex items-center justify-between gap-3 mb-4 pb-3 border-b border-white/[0.06] pl-2">
          ...
          <span className="text-[11px] tabular-nums">
            {typeof data.days_until_exam === 'number' && (
              <>
                <span className="font-bold" style={{ color: isCritical ? p.color : '#fff' }}>
                  J−{data.days_until_exam}
                </span>
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap pl-2">
          ...
          {data.lm && <span className="text-[10px] font-mono text-slate-400">{data.lm}</span>}
          {data.los && <span className="text-[10px] font-mono text-slate-500">· {data.los}</span>}
          {data.module_title && (
        ...
        {(data.mastery_pct != null || data.exam_weight_pct != null || data.urgency_score != null) && (
          <div className="grid grid-cols-3 gap-2 mb-5 pl-2">
        ...
        <div className="flex items-center justify-between gap-3 pt-3 border-t border-white/[0.04] pl-2">
          ...
          <button
            ...
            style={{
              background: p.color,
              boxShadow: p.pulse ? `0 0 16px ${p.color}55` : 'none',
            }}
          >
            Start session →
          </button>
...
export default NBA_Terminal_Card
